...
public class UcManageOrderImpl extends AbstractOrderUc implements UcManageOrder {
  ...
  public OrderCto saveOrderWithTwoPositionsAndOwner(OrderCto order) {
    ...
    OrderEntity entity = getBeanMapper().map(order, OrderEntity.class);
    return getBeanMapper().map(getOrderRepository().insertOrderWithTwoPositions(entity), OrderCto.class);
  }
}
