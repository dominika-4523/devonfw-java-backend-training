...
public class UcFindOrderImpl extends AbstractOrderUc implements UcFindOrder {
  ...
  public Set<OrderEto> findOrdersByDayAndStatus(LocalDate date, OrderStatus status) {
    ...
    return getOrderRepository().findOrdersByDayAndStatus(date, status).stream()
        .map(singleOrder -> getBeanMapper().map(singleOrder, OrderEto.class)).collect(Collectors.toSet());
  }
